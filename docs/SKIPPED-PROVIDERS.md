...
- Pi still ships first-party thin **`xai`** (~3 models). This repo now also ships **`xai-omp`** (full omp `xai`+`xai-oauth` catalog) — not a duplicate of Pi’s provider id.
...
| `xai-oauth` (as provider id) | Not registered under that id — use package provider **`xai-omp`** (ompAuthId `xai-oauth`, catalog `xai`∪`xai-oauth`) |
